...
static struct Userland_slab * create_slab(unsigned int pgs_per_slab,
					  ...
					  struct Objs_cache *cache_slab_descr,void* state);
static int destroy_slab(struct Userland_slab *slab, size_t slab_sz);
...
////////////////////New part //////////////////////////////////////////////
    ...
    void *new_slab_pgs = kmem_sys_malloc_specific(slab_sz,0,0&NK_ALLOC_ZERO);
...
void *objs_cache_alloc(struct Objs_cache *cache)
{
  ...
  if (cache != NULL) {
	...
	nk_vc_printf("Failed to allocate an object in %s (slab corrupted) !\n", __func__);
	return NULL;
      }
...
void objs_cache_free(struct Objs_cache *cache, void *obj)
{
  ...
  if (cache != NULL && obj != NULL) {
    ...
    if (slab == NULL) {
      nk_vc_printf("Failed to free an object in %s (slab corrupted) !\n", __func__);
    }
    ...
    */
    ...
    ERROR_PRINT("Error : cache NULL as parameter for %s\n", __func__);
    return;
  }
}
...
static void * __impl_alloc(void *state, size_t size, size_t align, int cpu, nk_alloc_flags_t flags){
    ...
    return objs_cache_alloc(&(as->cache));
...
static nk_alloc_interface_t slab_interface = {
   .destroy = destroy_slab,
   .allocp = impl_alloc,
   .reallocp = NULL,
   .freep = free_obj_from_slab,
  .print = print
 };

static struct nk_alloc * create(char *name){

    nk_vc_printf("Create Allocator--------  SLAB-------\n");

    slab_allocator_init();

    nk_vc_printf("Initila down--------------\n");

    struct nk_alloc_slab *as = kmem_sys_malloc_specific(sizeof(*as),my_cpu_id(),1);

    nk_vc_printf("After kmem_sys_malloc--------------\n");

    as->cache = objs_cache_init(&cache_Userland_slab,2,NULL);
    nk_vc_printf("After cache init--------------\n");

    as->alloc = nk_alloc_register(name,0,&slab_interface, as);

    nk_vc_printf("After alloc register-------------\n");
